...
# and `pip install classify_covid` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "classify_covid", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n classify_covid -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "classify_covid", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "classify_covid", "YOUR_CONSOLE_SCRIPT"]
